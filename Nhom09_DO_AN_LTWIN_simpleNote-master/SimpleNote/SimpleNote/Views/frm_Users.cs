...
namespace SimpleNote
{
    public partial class frm_Users : Form
    {
        ...
        {
            User _user = new User();
            _user.UserName = this.text_NameUser.Text;
            _user.Password = this.text_Password.Text;
            var flag = UserController.AddUser(_user);
            if(flag == true)
            {
                MessageBox.Show("Add user has be sucessfull ><");
            }
            else
            {
                MessageBox.Show("Error to add new user !!!!!");
            }
        }
    }
}
